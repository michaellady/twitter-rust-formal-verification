...
| `crates/store/src/lib.rs` | `verus_proof` (trusted skeleton — narrowed) | F6/F9 still documented as comments; `put_user`'s F3 dup-rejection (sub-PR 1) and `has_user`'s read-membership contract (sub-PR 2) are now actually discharged by Verus (see CHANGELOG-tier4.md). The remaining 5 methods (`put_follow`, `delete_follow`, `put_tweet`, `follow_set`, `home_timeline`) stay in the trusted skeleton until their respective sub-PRs land. | unit + integration + conformance tests |
| `crates/store/src/lib.rs` | `ExMemStore` external_type_specification (`external_body`) | Stream 3 Phase 4 sub-PR 1: `MemStore` is opaque to Verus — vstd 0.0.0-2026-04-20-1748 has no model of `std::sync::RwLock`, and the inner `HashMap<String, User>` lives behind that lock. Reasoned about exclusively through the `users_keys` ghost view + `proof_users_*` shims below. | tests |
| `crates/store/src/lib.rs` | `users_keys(s)` (closed spec, opaque) | Stream 3 Phase 4 sub-PR 1: ghost view of the set of currently-registered handles (each viewed as `Seq<char>`). Opaque because Verus cannot see through `RwLock` into the protected `HashMap`. The `proof_users_contains` / `proof_users_insert` shims chain through it. | unit tests asserting `put_user_then_has_user` / `put_user_duplicate_rejected` |
| `crates/store/src/lib.rs` | `proof_users_contains` (Stream 3 Phase 4, `external_body` exec shim) | Trusted spec wrapper around the lock-acquire + `HashMap::contains_key` step inside `MemStore::put_user`. Pins the returned `bool` to `users_keys(s).contains(handle@)` so `put_user_ensures` and (sub-PR 2) `has_user_ensures` can both chain through it. Body calls the real `s.inner.read().users.contains_key(handle)`; what is trusted is the spec on `RwLock::read`'s exclusivity + std `HashMap::contains_key`'s membership semantics. Retired when vstd ships `HashMap`/`RwLock` primitives whose own postconditions can be chained through directly. | unit tests; F3 TLC invariant |
