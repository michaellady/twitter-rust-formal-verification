...
| `crates/server/src/ui.rs` | entire module (Phase 2) | server-rendered HTMX UI on the primary backend; HMAC-signed actor cookie; routes `/`, `/_session`, `/u/:handle`, `/compose`, `/register`, `/_ui/follow`, `/_ui/unfollow` | manual smoke tested locally (login → home → compose → profile → follow); Phase 3 Playwright E2E will cover end-to-end |
